...
// ⬇️ Railway 배포 후 실제 URL로 교체하세요
// 예: "https://snsmakeit-yt-server-production.up.railway.app"
const YT_SERVER_URL = "";  // 비어있으면 서버 미연결 상태로 표시
...
export default function ShortformEditor({ isDark }) {
  ...
  const fetchYt = async (url) => {
    ...
    setYtLoading(true);
    try {
      // 서버가 연결된 경우 → 정확한 길이/정보 가져오기
      if (YT_SERVER_URL) {
        const r = await fetch(`${YT_SERVER_URL}/api/youtube-info?url=${encodeURIComponent(url)}`);
        if (r.ok) {
          const d = await r.json();
          setYtInfo({ id, title: d.title||url, thumbnail: d.thumbnail||`https://img.youtube.com/vi/${id}/hqdefault.jpg`, duration: d.duration });
          if (d.duration) { setTotalDuration(d.duration); setRangeEnd(d.duration); }
          setYtLoading(false); return;
        }
      }
      // 서버 없을 때 → oEmbed fallback
      const r = await fetch(`https://www.youtube.com/oembed?url=https://www.youtube.com/watch?v=${id}&format=json`);
      const d = await r.json();
      setYtInfo({ id, title: d.title||url, thumbnail:`https://img.youtube.com/vi/${id}/hqdefault.jpg`, duration: null });
    } catch {
  ...
  // 유튜브 구간 → 서버에서 세로영상 처리
  const processYoutubeSegment = async (clip) => {
    if (!YT_SERVER_URL) {
      alert("⚠️ 서버가 연결되지 않았어요.\n\nRailway에 youtube-server를 배포하고\nShortformEditor.jsx의 YT_SERVER_URL을 설정해주세요.");
      return;
    }
    if (!youtubeUrl || recording) return;
    const startSec = parseTimeToSec(clip.startTime);
    const endSec   = parseTimeToSec(clip.endTime);

    setRecording(true); setRecordPct(0);
    setProgressMsg("서버에서 유튜브 영상 다운로드 중...");

    try {
      // 진행률 시뮬레이션 (서버 작업 중)
      const dur = (endSec - startSec) || 30;
      const simInterval = setInterval(() => {
        setRecordPct(p => Math.min(p + 2, 88));
      }, (dur * 1000) / 50);

      const res = await fetch(`${YT_SERVER_URL}/api/youtube-process`, {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          startTime: startSec,
          endTime: endSec,
          clipTitle: clip.title_a || "shortform",
        }),
      });
      ...
      setRecordPct(90); setProgressMsg("영상 변환 완료, 저장 중...");

      // Blob으로 받아서 재생 가능한 URL 생성
      ...
      setExtractedVideos(prev => ({
        ...prev,
        [clipKey]: { url: blobUrl, blob, mimeType: "video/mp4", clip, isYoutube: true }
      }));
      setActivePreview(blobUrl);
      setRecordPct(100); setProgressMsg("");
    } catch(e) {
              ...
              {(() => {
                ...
                return <>
                  ...
                  {!isFile && (
                    <div>
                      {YT_SERVER_URL ? (
                        <button onClick={() => processYoutubeSegment(cur)} disabled={recording}
                          style={{width:"100%",padding:"9px",borderRadius:9,border:"none",cursor:recording?"not-allowed":"pointer",
                            background:recording?"rgba(168,85,247,0.2)":"linear-gradient(135deg,#a855f7,#ec4899)",
                            color:"#fff",fontSize:12,fontWeight:800,opacity:recording?0.6:1}}>
                          {recording ? `⏺ 처리 중 ${recordPct}%` : "✂️ 서버에서 세로영상 추출"}
                        </button>
                      ) : (
                        <div style={{padding:"10px",borderRadius:8,background:"rgba(245,158,11,0.08)",border:"1px solid rgba(245,158,11,0.2)",fontSize:10,color:"#fbbf24",textAlign:"center",lineHeight:1.7}}>
                          ⚠️ <b>서버 미연결</b><br/>
                          Railway에 youtube-server를 배포하고<br/>
                          <code style={{fontSize:9,color:"#f59e0b"}}>YT_SERVER_URL</code>을 설정해주세요.<br/>
                          <span style={{color:muted,fontSize:9}}>아래 배포 가이드를 참고하세요</span>
                        </div>
                      )}
                      {recording && (
                        <div style={{marginTop:6}}>
                          <div style={{height:4,borderRadius:2,background:"rgba(255,255,255,0.1)",overflow:"hidden",marginBottom:3}}>
                            <div style={{height:"100%",borderRadius:2,background:ACC,width:`${recordPct}%`,transition:"width 0.3s"}}/>
                          </div>
                          <div style={{fontSize:9,color:ACC,textAlign:"center"}}>{progressMsg}</div>
                        </div>
                      )}
                    </div>
                  )}
