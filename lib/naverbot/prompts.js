...
const NAVER_QUALITY_RULE = `
[네이버 블로그 품질 구조]
1. 도입부 직후 AEO 질문-답변 3개를 반드시 넣으세요.
   형식은 반드시 "Q. 질문" 다음 줄에 "A. 답변"입니다. Q와 A는 각각 독립된 줄에 작성하세요.
   "FAQ", "자주 묻는 질문"이라는 라벨은 절대 쓰지 마세요.
   Q1은 정의형, Q2는 방법형, Q3은 가격·비교·이유형으로 구성하세요.
   답변은 60~120자, 첫 문장에 결론을 작성하세요.
2. AEO 블록 다음에 핵심 정보 박스를 넣으세요.
...
3. 본문은 [SUBTITLE] 5~7개로 구성하고, 각 섹션은 250~400자 안팎으로 작성하세요.
4. 마지막에는 솔직 평가를 넣으세요. 반드시 아래 형식으로 작성하세요:
   장점: (장점 3가지를 쉼표로 구분)
   단점: (단점 2가지를 쉼표로 구분)
   경제·사회 이슈는 "긍정:" / "우려:" 로 바꿔도 됩니다.
5. 결론에는 반드시 아래 형식으로 작성하세요:
   추천 대상: (구체적 대상)
   비추천 대상: (구체적 대상)
6. 핵심 키워드는 본문 전체에서 5회 이내로 자연스럽게만 반복하세요.
7. 출처가 없는 수치, 기관명, 통계, 금리, 세율, 인용문은 만들지 마세요.
...
export function buildBlogPrompt(params) {
  ...
  const tail = speechRule + NAVER_QUALITY_RULE + CATEGORY_INPUT_RULE + NO_ENDING_RULE + NO_SPECIAL_RULE + TITLE_RULE + dateRule + (trendsText || "");
...
[참고 글 모드에서 추가 구조]
- 참고 글에 Q/A나 핵심 정보 박스가 없더라도, 도입부 직후 AEO Q/A 3개(Q. 질문 / A. 답변)를 추가하세요.
- AEO 블록 다음에 [TABLE] 마커와 5~7개 핵심 항목을 "항목: 값" 형식으로 넣으세요.
