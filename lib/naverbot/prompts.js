...
const IMAGE_RULE = `
[글 구조 필수 규칙]
1. 큰 소제목(SUBTITLE) → 핵심 요약 인용구(QUOTE) → 이미지 → 본문 설명 순서로 반복
2. 아래 마커를 본문 안에 정확히 삽입:
   [SUBTITLE] 큰 소제목 텍스트
   [QUOTE] 한 줄 핵심 요약 (소제목 내용의 후킹 문장)
   [image: english keyword]
   본문 단락...
3. 이 구조가 3~5번 반복되어 전체 글이 완성됨
4. [image: keyword]는 반드시 영문 2~3단어로, 사진 검색 시 정확히 해당 사물/장면이 나올 만큼 구체적
   좋은 예시: [image: glucose meter finger], [image: korean office desk]
   나쁜 예시: [image: health], [image: technology]
5. [QUOTE]는 반드시 한 문장, 강한 인상, 소제목 내용의 핵심 요약`;
...
출력 형식 (반드시 준수):
[TITLE]
(여기에 30~40자 호기심 후킹 제목 한 줄. 키워드를 그대로 반복하거나 주제를 단순 나열하지 말 것. 숫자, 감정 트리거, 반전 요소 활용. 네이버 블로그 검색 노출 + 클릭률을 극대화하는 SEO 후킹형 제목.)

[BODY]
(본문 시작. 본문 첫 줄에 제목을 또 쓰지 말 것 - 제목은 [TITLE]에만 있음.
본문 중간에 [image: english keyword] 마커 3~5개 자연스럽게 배치.)
(본문 끝)
...
== 중요 ==
- [TITLE]은 반드시 후킹 제목. "1인 사업자가 알아야 할 절세 팁" 같이 밋밋한 건 0점. "세무사도 안 알려주는 1인 사업자 절세 꿀팁, 연 300만원 아낄 수 있어" 같이 구체적 + 호기심 자극 + 숫자 포함이 100점.
- [BODY] 첫 줄에 제목을 다시 쓰지 말 것. 바로 도입부 문장부터 시작.
- [TAGS] 섹션 반드시 포함.`;

  return { system, user: body };
}

// 본문에서 [image: ...], [SUBTITLE] ..., [QUOTE] ... 마커 모두 분할
// 반환: [{type:"text"|"image"|"subtitle"|"quote", ...}, ...]
export function splitBodyByImageMarkers(body) {
  const blocks = [];
  // 통합 정규식: [image: xx], [SUBTITLE] xx, [QUOTE] xx 한 줄
