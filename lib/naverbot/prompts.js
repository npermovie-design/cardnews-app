...
소제목은 [SUBTITLE] 마커로만 표시.
예외: 강조할 핵심 문구는 **문구** 형식만 허용. 한 문단에 1개 이하로만 사용.`;
...
export function buildBlogPrompt(params) {
  ...
  const gifRule = useGif
    ? `\n[GIF 규칙]\n- 본문 중 자연스럽게 1~2개만 [gif: 한글 키워드] 마커를 넣으세요.\n- [gif:]는 가벼운 리액션이나 분위기 전환 지점에만 넣고, [image:]와 같은 줄에 쓰지 마세요.`
    : "";
  const imageRule = structurePattern + IMAGE_RULE_BASE + gifRule;
...
- GIF 사용 요청이 있으면 본문 중 1~2개 [gif: 한글 키워드] 마커를 넣으세요.
- [TAGS] 절대 빠뜨리지 마세요.
