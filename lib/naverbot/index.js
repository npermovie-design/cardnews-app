...
export async function verifyMakeitAccount(email, password) {
  if (!email || !password) return { ok: false, reason: "이메일/비밀번호 필수" };
  ...
  const { data: subs } = await supabase
    .from("subscriptions")
    .select("status, plan_id, ends_at, next_payment_at, updated_at")
    .eq("uid", uid)
  ...
  if (!activeSub) {
    return { ok: false, reason: "활성 구독이 없습니다. snsmakeit.com에서 구독 후 이용 가능" };
  }

  // 플랜명 정규화 (starter/pro/business) — 구독 plan_id 기반
  const planId = (activeSub.plan_id || "").toLowerCase();
  let plan = "starter";
  if (planId.includes("business") || planId.includes("enterprise")) plan = "business";
  else if (planId.includes("pro") || planId.includes("premium")) plan = "pro";
  ...
  return {
    ...
    plan,
    expires_at: activeSub.ends_at || activeSub.next_payment_at || null,
  };
}
