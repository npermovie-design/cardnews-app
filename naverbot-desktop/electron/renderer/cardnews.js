...
(function() {
    ...
    if (bgUrl) {
      try {
        fabric.Image.fromURL(bgUrl, function(img) {
          if (!img || !img.width) { addTexts(); return; }
          img.scaleToWidth(W);
        ...
        console.warn("[cardnews] 배경 이미지 로드 실패:", imgErr.message);
        addTexts();
      }
    } else {
      addTexts();
  ...
  $("cardUrlFetchBtn")?.addEventListener("click", async function() {
    ...
    if (!/^https?:\/\//i.test(trimmed)) { showModal("알림", "http:// 또는 https://로 시작하는 URL을 입력해주세요.", "확인"); return; }
    $("cardUrlFetchBtn").textContent = "가져오는 중..."; $("cardUrlFetchBtn").disabled = true;
    try {
      var controller = new AbortController();
      var timeout = setTimeout(function() { controller.abort(); }, 15000);
      var res = await fetch(API + "/fetch-url-content", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ url: trimmed }), signal: controller.signal });
      ...
      if (_cardRefContent) {
        ...
        showModal("알림", "해당 URL에서 텍스트를 추출하지 못했습니다.", "확인");
      }
    } catch (e) {
      showModal("오류", e.name === "AbortError" ? "요청 시간이 초과되었습니다." : "URL을 가져올 수 없습니다.", "확인");
    }
  ...
  // ─── 횟수 관리: 카드뉴스도 콘텐츠 생성 1회로 통합 차감 ───
  async function checkCardLimit() {
    if (typeof checkWriteLimit === "function") return checkWriteLimit();
    return { canUse: false, used: 0, limit: 0, remaining: 0 };
  }
  async function markCardUsed() {
    if (typeof markFeatureUsed === "function") return markFeatureUsed("write", "데스크톱 카드뉴스 생성");
  ...
  $("cardGenerateBtn")?.addEventListener("click", async function() {
    ...
    if (!quota.canUse) return showModal("한도 초과", "이번 달 콘텐츠 생성 한도(" + quota.limit + "회)를 사용했습니다.", "구독하기", function() { bridge.openExternal("https://snsmakeit.com/pricing"); });
    ...
    try {
      ...
      if (!jsonMatch) throw new Error("슬라이드 생성 실패 - AI 응답에서 JSON을 찾지 못했습니다.");
      var parsed;
      try { parsed = JSON.parse(jsonMatch[0]); } catch (pe) { throw new Error("슬라이드 JSON 파싱 실패: " + pe.message); }
