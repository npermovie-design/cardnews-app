...
if (!bridge) {
  document.body.innerHTML =
    '<div style="padding:40px;color:#7c6aff;font-family:sans-serif;">' +
...
function renderPlanCard(data) {
  ...
  if (data.trial) {
    ...
    card.innerHTML = `
      ...
      <div class="plan-desc">남은 횟수 <strong style="color:#7c6aff;">${remaining}회</strong> / ${data.trial_limit}회 · ${escapeHtml(rules.desc)}</div>
      ${experienceText}
    ...
    el.innerHTML = sorted.map((m) => {
      ...
      return `<div style="margin-bottom:12px;display:flex;flex-direction:column;align-items:${isMe ? "flex-end" : "flex-start"};">
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:3px;">
          <span style="font-size:12px;font-weight:700;color:${isMe ? "#7c6aff" : "#333"};">${m.nick || m.email?.split("@")[0] || "익명"}</span>
          <span style="font-size:10px;color:#bbb;">${time}</span>
          ${canDelete ? `<button onclick="deleteChat('${m.id}')" style="font-size:10px;color:#ef4444;background:none;border:none;cursor:pointer;">삭제</button>` : ""}
        </div>
        <div style="max-width:80%;padding:10px 14px;border-radius:${isMe ? "14px 14px 4px 14px" : "14px 14px 14px 4px"};background:${isMe ? "#7c6aff" : "#f3f4f6"};color:${isMe ? "#fff" : "#333"};font-size:13px;line-height:1.6;word-break:break-word;">
...
if (feedbackCatChips) {
  feedbackCatChips.addEventListener("click", (e) => {
    ...
    feedbackCatChips.querySelectorAll("button").forEach((b) => {
      ...
      b.style.border = active ? "1.5px solid #7c6aff" : "1px solid #e5e7eb";
      b.style.background = active ? "#7c6aff15" : "transparent";
      b.style.color = active ? "#7c6aff" : "#888";
