...
export default function DetailPageGenerator({ isDark, user , onUserUpdate}) {
  ...
  const accentColor = selCat ? cat.accent : "#7c6aff";
                ...
                {steps.map((s,i)=>(
                  <div key={i} style={{display:"flex",alignItems:"center",gap:10,opacity:s.done||s.active?1:0.3,transition:"opacity 0.3s"}}>
                    <div style={{width:22,height:22,borderRadius:"50%",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,
                      ...
                      border:s.done?"2px solid #4ade80":s.active?"2px solid #7c6aff":"2px solid rgba(255,255,255,0.1)",transition:"all 0.3s"}}>
              ...
              {progress.total > 0 && (
                <div style={{ maxWidth:280,width:"100%",margin:"0 auto 20px" }}>
                  <div style={{ height:6,borderRadius:4,background:isDark?"rgba(255,255,255,0.08)":"rgba(0,0,0,0.06)",overflow:"hidden",marginBottom:6 }}>
                    <div style={{ height:"100%",borderRadius:4,background:`linear-gradient(90deg,${accentColor},#8b5cf6,#1a1730)`,width:`${Math.max(5,(progress.cur/progress.total)*100)}%`,transition:"width 0.5s ease" }}/>
                  </div>
