...
function AiSidebar({ aiMenu, setAiMenu, user, onQna, theme, onlineCount, navigate, onLogout, onCollapse }) {
  ...
  const menuLabel= isDark ? "rgba(255,255,255,0.2)"      : "rgba(59,130,246,0.36)";
  const itemText = isDark ? "rgba(255,255,255,0.5)"      : "rgba(17,24,39,0.62)";
  const itemActive= isDark ? "#60a5fa"                   : "#3b82f6";
  const itemActiveBg = isDark ? "rgba(59,130,246,0.24)" : "rgba(59,130,246,0.08)";
  const brandText= isDark ? "#fff"                       : "#111827";
  const brandSub = isDark ? "rgba(255,255,255,0.3)"      : "rgba(59,130,246,0.52)";
  const comText  = isDark ? "rgba(255,255,255,0.45)"     : "#888";
  const usageBar = isDark ? "rgba(255,255,255,0.08)"     : "rgba(59,130,246,0.12)";
  ...
  const Item = ({ id, label, icon, ids, badge, badgeColor }) => {
    ...
    return (
      ...
      onMouseEnter={e => { if (!active) e.currentTarget.style.background = "rgba(59,130,246,0.06)"; }}
  ...
  return (
    <div style={{
      ...
      <div style={{ padding:"10px", borderTop:`1px solid ${sideBdr}`, flexShrink:0 }}>
        <button onClick={() => navigate && navigate("mypage")}
          style={{
            ...
            color: isDark ? "#60a5fa" : "#2563eb", fontSize:14, fontWeight:700,
      ...
      <div ref={dragRef} onMouseDown={handleMouseDown}
        ...
        onMouseEnter={e=>e.currentTarget.style.background=isDark?"rgba(59,130,246,0.3)":"rgba(59,130,246,0.15)"}
        onMouseLeave={e=>e.currentTarget.style.background="transparent"} />
    </div>
  );
}
...
function SidebarProfile({ user, info, freeLimit, pct, isDark, sideBdr, navigate, onLogout, onlineCount, usageText, usageBar, t }) {
  ...
  const ptColor = isEmpty || isLow ? "#f87171" : "#60a5fa";
    ...
    <div style={{ position: "relative" }}>
      ...
      {open && (
        <div style={{
          ...
          border:`1px solid ${isDark?"rgba(59,130,246,0.25)":"rgba(59,130,246,0.18)"}`,
          borderRadius:16, overflow:"hidden",
          boxShadow: isDark
            ? "0 8px 32px rgba(0,0,0,0.5), 0 0 0 1px rgba(59,130,246,0.1)"
            : "0 8px 32px rgba(59,130,246,0.12), 0 2px 8px rgba(0,0,0,0.08)",
          backdropFilter:"blur(20px)",
        }}>
          {/* 헤더 */}
          <div style={{ padding:"16px 16px 12px",
            background: isDark?"rgba(59,130,246,0.08)":"rgba(59,130,246,0.04)",
            borderBottom:`1px solid ${isDark?"rgba(255,255,255,0.06)":"rgba(59,130,246,0.1)"}` }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ display:"flex", alignItems:"center", gap:5, marginBottom:2 }}>
                  ...
                  <span style={{ fontSize:9, padding:"1px 5px", borderRadius:4, fontWeight:700,
                    background: user.role==="admin"?"rgba(251,191,36,0.15)":"rgba(59,130,246,0.12)",
                    color: user.role==="admin"?"#fbbf24":"#60a5fa", flexShrink:0 }}>
          ...
          {/* 잔여 횟수 */}
          <div style={{ padding:"12px 16px", borderBottom:`1px solid ${isDark?"rgba(255,255,255,0.06)":"rgba(59,130,246,0.08)"}` }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
              <span style={{ fontSize:11, color:muted }}>잔여 횟수</span>
              <span style={{ fontSize:14, fontWeight:900,
                color: isEmpty?"#f87171":isLow?"#f59e0b":"#60a5fa" }}>
                {usesLeft}회
              </span>
            </div>
            <div style={{ height:4, borderRadius:4, background:isDark?"rgba(255,255,255,0.08)":"rgba(59,130,246,0.1)", overflow:"hidden", marginBottom:6 }}>
              <div style={{ height:"100%", borderRadius:4,
                background: isEmpty?"#f87171":isLow?"linear-gradient(90deg,#f59e0b,#fbbf24)":"#3b82f6",
            ...
            {[
              ...
              <button key={i} onClick={item.onClick}
                ...
                <div style={{ width:28, height:28, borderRadius:8, flexShrink:0,
                  background: item.accent?"#3b82f6":isDark?"rgba(255,255,255,0.06)":"rgba(59,130,246,0.08)",
                  display:"flex", alignItems:"center", justifyContent:"center", fontSize:13 }}>
                  {item.accent?"+":"L"}
                </div>
                <div>
                  <div style={{ fontSize:13, fontWeight:600, color:item.accent?"#60a5fa":text }}>{item.label}</div>
      ...
      <button onClick={() => setOpen(p => !p)}
        style={{ width:"100%", padding:"11px 14px",
          background: open?(isDark?"rgba(59,130,246,0.1)":"rgba(59,130,246,0.06)"):"transparent",
          border:"none", cursor:"pointer",
          display:"flex", alignItems:"center", gap:10,
          borderTop:`1px solid ${bdr}`,
          transition:"background 0.15s" }}
        onMouseEnter={e=>{ if(!open) e.currentTarget.style.background=isDark?"rgba(255,255,255,0.04)":"rgba(59,130,246,0.04)"; }}
