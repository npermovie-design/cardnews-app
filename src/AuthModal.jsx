...
export default function AuthModal({ onClose, onAuth, C, embedded = false }) {
  ...
  const googleLogin = async () => {
    ...
    try {
      ...
      setErr(ko ? "구글 로그인 중 오류가 발생했습니다." : "An error occurred during Google login.");
      setLoading(false);
    }
  };
  ...
  const login = async () => {
    setErr(""); setLoading(true);
    try {
      if (!form.email || !form.pw) { setErr(ko ? "이메일과 비밀번호를 입력해주세요." : "Please enter your email and password."); setLoading(false); return; }
      if (!isValidEmail(form.email)) { setErr(ko ? "올바른 이메일 형식이 아닙니다." : "Invalid email format."); setLoading(false); return; }
      ...
      const msg = (e.message?.includes("Invalid login") || e.message?.includes("invalid_credentials") || e.message?.includes("Invalid email"))
        ? (ko ? "이메일 또는 비밀번호가 올바르지 않습니다." : "Invalid email or password.")
        : e.message?.includes("Email not confirmed")
        ? (ko ? "이메일 인증이 완료되지 않았습니다. 메일함을 확인해주세요." : "Email not verified. Please check your inbox.")
        : e.message?.includes("too many requests") || e.message?.includes("rate limit")
        ? (ko ? "로그인 시도가 너무 많습니다. 잠시 후 다시 시도해주세요." : "Too many login attempts. Please try again later.")
        : e.message?.includes("시간이 초과")
        ? e.message
        : e.message?.includes("User not found") || e.message?.includes("user not found")
        ? (ko ? "가입되지 않은 이메일입니다. 회원가입을 먼저 해주세요." : "Email not registered. Please sign up first.")
        : (ko ? `오류: ${e.message}` : `Error: ${e.message}`);
      setErr(msg);
  ...
  const register = async () => {
    setErr(""); setLoading(true);
    try {
      if (!form.nick.trim() || form.nick.trim().length < 2) { setErr(ko ? "닉네임을 2자 이상 입력해주세요." : "Nickname must be at least 2 characters."); setLoading(false); return; }
      if (!isValidEmail(form.email)) { setErr(ko ? "올바른 이메일 형식이 아닙니다." : "Invalid email format."); setLoading(false); return; }
      ...
      const msg = e.message?.includes("already registered") || e.message?.includes("already been registered") || e.message?.includes("User already registered")
        ? (ko ? "이미 가입된 이메일입니다." : "This email is already registered.")
        : e.message?.includes("Password should")
        ? (ko ? "비밀번호가 너무 약합니다. (8자 이상)" : "Password is too weak. (8+ characters)")
        : e.message?.includes("valid email")
        ? (ko ? "올바른 이메일 형식이 아닙니다." : "Invalid email format.")
        : e.message?.includes("시간이 초과")
        ? e.message
        : (ko ? `오류: ${e.message}` : `Error: ${e.message}`);
      setErr(msg);
  ...
  const checkVerification = async () => {
    ...
    try {
      ...
      setErr(ko ? "아직 인증이 완료되지 않았어요. 이메일 링크를 클릭해주세요." : "Verification not yet complete. Please click the link in your email.");
  ...
  const resendEmail = async () => {
    ...
    } catch(e) { setErr(ko ? "재발송 중 오류가 발생했어요." : "Error resending email."); }
  };
    ...
    <div onClick={embedded ? undefined : onClose} style={embedded ? {} : { position: "fixed", inset: 0, zIndex: 9999, background: "rgba(26,23,48,0.45)", display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(8px)" }}>
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: "rgba(18,16,58,0.98)", border: "1px solid rgba(0,0,0,0.06)", borderRadius: 22, padding: "clamp(24px,5vw,36px) clamp(18px,4vw,30px)", width: "100%", maxWidth: 420, position: "relative", boxShadow: embedded ? "0 8px 32px rgba(0,0,0,0.2)" : "0 24px 64px rgba(0,0,0,0.3)", margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", marginBottom: 24, background: C.toggleBg, borderRadius: 10, padding: 4 }}>
          {[["login", ko ? "로그인" : "Login"],["register", ko ? "회원가입" : "Sign up"]].map(([t, l]) => (
        ...
        {tab === "login" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <input placeholder={ko ? "비밀번호" : "Password"} type="password" value={form.pw} className="nper-auth-input" style={fs} onChange={f("pw")} onKeyDown={e => e.key === "Enter" && login()} />
            ...
            {err === "__inapp__" && (
              <div style={{ borderRadius: 12, border: "1px solid rgba(245,158,11,0.3)", background: "rgba(245,158,11,0.08)", padding: "14px 16px" }}>
                <div style={{ fontSize: 13, color: "#f59e0b", fontWeight: 700, marginBottom: 8 }}>{ko ? "카카오 앱에서는 구글 로그인이 불가해요" : "Google login is not available in the Kakao app"}</div>
                <div style={{ fontSize: 12, color: "rgba(255,255,255,0.6)", lineHeight: 1.8, marginBottom: 12 }}>
                  {ko ? <>Google 정책상 카카오·네이버 등 앱 내 브라우저에서는 구글 로그인이 차단됩니다.<br/><b style={{ color: "#fff" }}>Chrome 브라우저</b>에서 직접 접속해 로그인해주세요.</> : <>Google blocks login from in-app browsers (Kakao, Naver, etc.).<br/>Please open in <b style={{ color: "#fff" }}>Chrome browser</b> to log in.</>}
                </div>
                <button onClick={openInExternalBrowser}
                  ...
                  {ko ? "Chrome으로 열기" : "Open in Chrome"}
                </button>
                <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", textAlign: "center", marginTop: 8 }}>
                  {ko ? <>또는 주소창에 <b style={{ color: "rgba(255,255,255,0.6)" }}>snsmakeit.com</b> 직접 입력</> : <>Or type <b style={{ color: "rgba(255,255,255,0.6)" }}>snsmakeit.com</b> in your address bar</>}
                </div>
              </div>
            )}
            <button onClick={login} disabled={loading} style={{ padding: "13px", borderRadius: 12, border: "none", cursor: loading ? "not-allowed" : "pointer", background: loading ? "rgba(0,0,0,0.06)" : "#3b82f6", color: "#fff", fontSize: 14, fontWeight: 700, minHeight: 44 }}>
              {loading ? (ko ? "로그인 중..." : "Logging in...") : (ko ? "로그인하기" : "Login")}
            </button>
            <div style={{ display: "flex", alignItems: "center", gap: 8, margin: "4px 0" }}>
              <div style={{ flex: 1, height: 1, background: "rgba(255,255,255,0.1)" }} />
              <span style={{ fontSize: 11, color: "rgba(255,255,255,0.3)" }}>{ko ? "또는" : "or"}</span>
              <div style={{ flex: 1, height: 1, background: "rgba(255,255,255,0.1)" }} />
            </div>
            <button onClick={googleLogin} disabled={loading}
              style={{ padding: "12px", borderRadius: 12, border: "1px solid rgba(255,255,255,0.15)", cursor: "pointer", background: "rgba(255,255,255,0.06)", color: "#fff", fontSize: 13, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, minHeight: 44 }}>
              ...
              {ko ? "Google로 로그인" : "Login with Google"}
            </button>
            <button onClick={() => alert(ko ? "npermovie@naver.com으로 문의해주세요" : "Please contact npermovie@naver.com")} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.4)", fontSize: 12, textAlign: "center", padding: "2px 0 0" }}>
            ...
            <button onClick={() => { setTab("register"); setErr(""); }} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.4)", fontSize: 12, textAlign: "center", padding: "4px" }}>
              {ko ? <>아직 계정이 없으신가요? <span style={{ color: "#a5b4fc", fontWeight: 700 }}>회원가입</span></> : <>Don't have an account? <span style={{ color: "#a5b4fc", fontWeight: 700 }}>Sign up</span></>}
            </button>
          </div>
        )}
        ...
        {tab === "register" && regStep === 1 && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <input placeholder={ko ? "닉네임 (2자 이상)" : "Nickname (2+ chars)"} value={form.nick} className="nper-auth-input" style={fs} onChange={f("nick")} />
            <input placeholder={ko ? "이메일" : "Email"} type="email" value={form.email} className="nper-auth-input" style={fs} onChange={f("email")} />
            ...
            {err === "__inapp__" && (
              <div style={{ borderRadius: 12, border: "1px solid rgba(245,158,11,0.3)", background: "rgba(245,158,11,0.08)", padding: "14px 16px" }}>
                <div style={{ fontSize: 13, color: "#f59e0b", fontWeight: 700, marginBottom: 8 }}>{ko ? "카카오 앱에서는 구글 로그인이 불가해요" : "Google login is not available in the Kakao app"}</div>
                <div style={{ fontSize: 12, color: "rgba(255,255,255,0.6)", lineHeight: 1.8, marginBottom: 12 }}>
                  {ko ? <>Google 정책상 카카오·네이버 등 앱 내 브라우저에서는 구글 로그인이 차단됩니다.<br/><b style={{ color: "#fff" }}>Chrome 브라우저</b>에서 직접 접속해 로그인해주세요.</> : <>Google blocks login from in-app browsers (Kakao, Naver, etc.).<br/>Please open in <b style={{ color: "#fff" }}>Chrome browser</b> to sign up.</>}
                </div>
                <button onClick={openInExternalBrowser}
                  style={{ width: "100%", padding: "10px", borderRadius: 9, border: "none", cursor: "pointer", background: "linear-gradient(135deg,#f59e0b,#f97316)", color: "#fff", fontSize: 13, fontWeight: 700 }}>
                  {ko ? "Chrome으로 열기" : "Open in Chrome"}
            ...
            <label style={{ display: "flex", alignItems: "flex-start", gap: 8, cursor: "pointer", padding: "4px 0" }}>
              <input type="checkbox" checked={agreedToTerms} onChange={e => setAgreedToTerms(e.target.checked)}
                style={{ marginTop: 2, accentColor: "#3b82f6", width: 16, height: 16, flexShrink: 0, cursor: "pointer" }} />
            ...
            <button onClick={register} disabled={loading || !agreedToTerms} style={{ padding: "13px", borderRadius: 12, border: "none", cursor: (loading || !agreedToTerms) ? "not-allowed" : "pointer", background: (loading || !agreedToTerms) ? "rgba(0,0,0,0.06)" : "#3b82f6", color: "#fff", fontSize: 14, fontWeight: 700, opacity: !agreedToTerms ? 0.5 : 1, minHeight: 44 }}>
        ...
        {tab === "register" && regStep === 2 && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14, textAlign: "center" }}>
            {/* 아이콘 */}
            <div style={{ fontSize: 18, fontWeight: 700, color: "#3b82f6", margin: "8px 0 4px" }}>{ko ? "이메일 인증" : "Email Verification"}</div>
            ...
            <div style={{ fontSize: 13, color: "rgba(255,255,255,0.55)", lineHeight: 1.9 }}>
              {ko ? <>
                ...
                <b style={{ color: "#a5b4fc" }}>{form.email}</b>.<br/>
                Please check your inbox, click the link,<br/>then press the button below.
              </>}
            </div>
            ...
            <div style={{ background: "rgba(0,0,0,0.06)", border: "1px solid rgba(0,0,0,0.06)", borderRadius: 12, padding: "12px 14px", textAlign: "left" }}>
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", lineHeight: 1.8 }}>
                {ko ? <>
                  ...
                  If you don't see the email, check your <b style={{ color: "#fff" }}>spam folder</b><br/>
                  After clicking the link, come back here and press the button below
                </>}
              </div>
            </div>
            ...
            <button onClick={checkVerification} disabled={verifyLoading}
              style={{ padding: "13px", borderRadius: 12, border: "none", cursor: verifyLoading ? "not-allowed" : "pointer", background: verifyLoading ? "rgba(0,0,0,0.06)" : "#3b82f6", color: "#fff", fontSize: 14, fontWeight: 800 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={resendEmail} disabled={resendCool}
                ...
                {resendCool ? (ko ? `${resendCount}초 후 재발송` : `Resend in ${resendCount}s`) : (ko ? "메일 재발송" : "Resend email")}
              </button>
