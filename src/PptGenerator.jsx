...
JSON만: {"outline":[{"no":1,"title":"표지","layout":"title_only","desc":"주제 및 발표자 소개"}]}`, 1500);
  ...
  const generate = async () => {
    ...
    try {
      const pptPrompt = `PPT 슬라이드 전문 기획자. 이모지 사용 금지.
...
JSON만: {"slides":[...]}`;
      // PPT 전용 Serverless 엔드포인트 사용 (60초 타임아웃)
      const pptRes = await fetch("/api/ai-ppt", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          model: "claude-sonnet-4-5",
          messages: [{ role: "user", content: pptPrompt }],
          max_tokens: Math.max(slideCount * 400, 5000),
        }),
      });
      if (!pptRes.ok) {
        const errText = await pptRes.text().catch(() => pptRes.statusText);
        throw new Error(`AI API 오류 ${pptRes.status}: ${errText}`);
      }
      const pptData = await pptRes.json();
      const r = pptData.choices?.[0]?.message?.content || "";
      ...
      <div style={{ maxWidth:660, margin:"0 auto" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:18 }}>
          <div>
            ...
            <input type="range" min={5} max={15} value={slideCount} onChange={e=>setSlideCount(Number(e.target.value))} style={{ width:"100%", accentColor:accent }} />
