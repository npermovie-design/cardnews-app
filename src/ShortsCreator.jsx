  ...
  return (
    <div style={{ background: "#1e1e3a", borderRadius: 10, padding: 12, marginBottom: 10 }}>
      ...
      <div style={{ display: "flex", gap: 0, marginBottom: 8, borderRadius: 6, overflow: "hidden", border: "1px solid #2a2a4a" }}>
        {CATS.map(([k,l]) => (
          <button key={k} onClick={() => { setCat(k); setSearch(""); }}
            style={{ flex: 1, padding: "5px 4px", border: "none", background: cat === k ? "#3b82f6" : "#12122a", color: cat === k ? "#fff" : "#666", cursor: "pointer", fontSize: 10, fontWeight: 700, transition: "all .15s" }}>{l}</button>
        ))}
      </div>
      {/* 검색창 */}
      <div style={{ display: "flex", gap: 4, marginBottom: 8 }}>
        ...
        {cat !== "archive" && <button onClick={() => doSearch()} style={{ padding: "4px 10px", borderRadius: 6, border: "1px solid #2a2a4a", background: "#12122a", color: "#3b82f6", cursor: "pointer", fontSize: 11, fontWeight: 700 }}>{t("sc_search_btn")}</button>}
      </div>
      {loading ? (
          ...
          {filteredItems.slice(0, 30).map((it, i) => (
            <div key={i} onClick={() => onSelect(it.videoUrl || it.url)}
              ...
              <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "linear-gradient(transparent,rgba(0,0,0,0.85))", padding: "8px 4px 2px", fontSize: 8, color: "#ddd", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                <span style={{ color: cat === "gif" ? "#ec4899" : cat === "video" ? "#4ade80" : "#3b82f6", marginRight: 3, fontSize: 7 }}>{it.src}</span>
                {it.title}
              </div>
            </div>
          ))}
...
export default function ShortsCreator({ isDark, user, onUserUpdate, onLoginRequest, setAiMenu, onStatusChange, showPointConfirm, initialVideoFile, initialVideoLink }) {
  ...
  // 하단 제목 스타일
  const [bottomTitleStyle, setBottomTitleStyle] = useState({ color: "#FFFFFF", fontSize: 22, shadow: true, border: false, borderColor: "#000", bgBox: true, bgColor: "rgba(0,0,0,0.7)", opacity: 100, font: "default", align: "center", bold: false, highlightColor: "#3b82f6" });
  // 자막 스타일
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: "#16162a", border: "1px solid #2a2a4a", borderRadius: 16, padding: "24px 28px", maxWidth: 420, width: "90%", maxHeight: "80vh", overflowY: "auto" }}>
            ...
            {[
              ...
              <div key={group} style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "#3b82f6", marginBottom: 6, textTransform: "uppercase" }}>{group}</div>
                {items.map(([key, desc]) => (
                  <div key={key} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "5px 0", borderBottom: "1px solid #1a1a30" }}>
                    ...
                    <kbd style={{ padding: "2px 8px", borderRadius: 4, background: "#0f0f1a", border: "1px solid #2a2a4a", color: "#3b82f6", fontSize: 11, fontFamily: "monospace", fontWeight: 600 }}>{key}</kbd>
                  </div>
                ))}
              </div>
            ))}
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", background: "#0f0f1a", padding: 40 }}>
          <div style={{ textAlign: "center", maxWidth: 320 }}>
            <div style={{ fontSize: 48, marginBottom: 16, opacity: 0.3 }}>
              <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#3b82f6" strokeWidth="1.5"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>
            </div>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden", minHeight: 0 }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "0 8px 8px" }}>
            {editClips.map((c, i) => (
              <div key={i} onClick={() => { setEditIdx(i); setSelectedSubIdx(-1); setPlayhead(0); setIsPlaying(false); }}
                style={{ padding: "10px 10px", borderRadius: 8, cursor: "pointer", marginBottom: 4, background: editIdx === i ? "rgba(0,0,0,0.06)" : "transparent", borderLeft: `3px solid ${editIdx === i ? "#3b82f6" : "transparent"}`, transition: "all 0.15s", display: "flex", alignItems: "center", gap: 6 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: editIdx === i ? "#3b82f6" : "#ccc", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.title || c.hook || c.bottom_title || `Short ${i + 1}`}</div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "#0c0c1a", padding: 12, minWidth: 0, overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", gap: 4, marginBottom: 8, flexShrink: 0 }}>
            {[["bars",t("sc_bars")],["full",t("sc_fullscreen")]].map(([k,l]) => (
              <button key={k} onClick={() => setLayoutMode(k)}
                style={{ padding: "4px 12px", borderRadius: 6, border: layoutMode===k ? "1px solid #3b82f6" : "1px solid #2a2a4a", background: layoutMode===k ? "rgba(0,0,0,0.06)" : "#1a1a30", color: layoutMode===k ? "#a5b4fc" : "#666", cursor: "pointer", fontSize: 10, fontWeight: 700 }}>{l}</button>
            ))}
          </div>
          ...
          <div ref={previewRef} style={{ width: 360, maxWidth: "100%", aspectRatio: "9/16", maxHeight: "calc(100% - 40px)", borderRadius: 8, background: "#000", border: "2px solid #2a2a4a", overflow: "hidden", boxShadow: "0 8px 40px rgba(0,0,0,0.6)", flexShrink: 0, flexGrow: 0, position: "relative", userSelect: "none" }}>
            ...
            {autoMediaLoading && (
              <div style={{ position: "absolute", inset: 0, zIndex: 50, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(4px)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, borderRadius: 6 }}>
                <div style={{ width: 48, height: 48, borderRadius: 16, background: "#3b82f6", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 20px rgba(0,0,0,0.06)" }}>
                  {autoMediaStep === "done"
                ...
                <div style={{ width: "70%", height: 6, borderRadius: 3, background: "rgba(255,255,255,0.1)", overflow: "hidden" }}>
                  <div style={{ height: "100%", borderRadius: 3, background: autoMediaStep === "done" ? "#4ade80" : "#3b82f6", width: `${autoMediaProgress}%`, transition: "width 0.3s ease" }} />
                </div>
            ...
            <div onMouseDown={e => handlePreviewMouseDown("title", e)}
              style={{ position: "absolute", left: `${titlePos.x}%`, top: `${titlePos.y}%`, transform: "translate(-50%,-50%)", cursor: "move", zIndex: 12, maxWidth: "90%", textAlign: titleStyle.align || "center", opacity: titleStyle.opacity / 100, border: dragging === "title" ? "2px solid #3b82f6" : "2px solid transparent", borderRadius: 8, padding: "2px" }}>
              <span style={{
            ...
            {/* 스냅 가이드라인 */}
            {snapGuide && (<>
              {(snapGuide.axis === "x" || snapGuide.axis === "both") && <div style={{ position: "absolute", left: `${snapGuide.pos || 50}%`, top: 0, width: 1, height: "100%", background: "#3b82f680", zIndex: 30, pointerEvents: "none" }} />}
              {(snapGuide.axis === "y" || snapGuide.axis === "both") && <div style={{ position: "absolute", top: `${snapGuide.y || snapGuide.pos}%`, left: 0, width: "100%", height: 1, background: "#3b82f680", zIndex: 30, pointerEvents: "none" }} />}
            </>)}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 10 }}>
            ...
            <button onClick={() => setIsPlaying(!isPlaying)} style={{ width: 40, height: 40, borderRadius: "50%", border: "none", background: "#3b82f6", color: "#fff", cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
              {isPlaying ? "⏸" : "▶"}
            </button>
          ...
          <div style={{ display: "flex", borderBottom: "1px solid #2a2a4a" }}>
            {[["style",t("sc_style_tab")],["overlay",t("sc_overlay_tab")]].map(([k,l]) => (
              <button key={k} onClick={() => setPropTab(k)}
                style={{ flex: 1, padding: "10px 0", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 700, background: propTab === k ? "#1e1e3a" : "transparent", color: propTab === k ? "#3b82f6" : "#666", borderBottom: propTab === k ? "2px solid #3b82f6" : "2px solid transparent" }}>{l}</button>
            ))}
          </div>
          ...
          <div style={{ flex: 1, padding: "10px 14px 14px", overflowY: "auto" }}>
            {propTab === "style" ? (<>
              ...
              {selectedSubIdx >= 0 && (curClip.subtitles || [])[selectedSubIdx] && (
                <div style={{ background: "#1e1e3a", borderRadius: 10, padding: 12, marginBottom: 10, border: "1px solid #3b82f640" }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#3b82f6", marginBottom: 8 }}>{t("sc_subtitle_on")} #{selectedSubIdx + 1}</div>
              ...
              {[["title","상단 제목 스타일",titleStyle,setTitleStyle,"#3b82f6"],["bottomTitle","하단 제목 스타일",bottomTitleStyle,setBottomTitleStyle,"#f59e0b"],["caption",t("sc_lower_caption"),captionStyle,setCaptionStyle,"#22d3ee"]].map(([key,label,st,setSt,ac]) => (
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 4 }}>
                  {TEMPLATES.map(tpl => (
                    <div key={tpl.id} onClick={() => { setTemplate(tpl.id); setTitleStyle(p=>({...p,color:tpl.titleColor})); setCaptionStyle(p=>({...p,color:tpl.captionColor})); }}
                      style={{ border: `2px solid ${template === tpl.id ? "#3b82f6" : "#2a2a4a"}`, borderRadius: 6, padding: 2, cursor: "pointer", textAlign: "center" }}>
                      <div style={{ height: 24, borderRadius: 4, background: tpl.bg }} />
                      <div style={{ fontSize: 7, fontWeight: 700, color: template === tpl.id ? "#3b82f6" : "#555", marginTop: 2 }}>{t(tpl.nameKey)}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div style={{ flexShrink: 0, background: "#111120", borderTop: "1px solid rgba(255,255,255,0.06)", display: "flex", flexDirection: "column", position: "relative" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "8px 16px", gap: 16, flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <button onClick={() => setIsPlaying(!isPlaying)} style={{ width: 44, height: 44, borderRadius: 22, border: "none", background: isPlaying ? "rgba(255,255,255,0.12)" : "#3b82f6", color: "#fff", cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: isPlaying ? "none" : "0 2px 12px rgba(0,0,0,0.06)" }}>
              {isPlaying
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 4, background: "rgba(255,255,255,0.04)", borderRadius: 8, padding: "4px 8px" }}>
              ...
              <input type="range" min="50" max="400" value={timelineZoom * 100} onChange={e => setTimelineZoom(Number(e.target.value)/100)} style={{ width: 60, accentColor: "#3b82f6", height: 3 }} />
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "0 16px 8px", flexShrink: 0, minHeight: 36 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 4, background: "rgba(255,255,255,0.04)", borderRadius: 8, padding: "4px 10px" }}>
            ...
            <input type="range" min="0" max="200" value={volume} onChange={e => setVolume(Number(e.target.value))} style={{ width: 60, accentColor: volume > 100 ? "#f59e0b" : "#3b82f6", height: 3 }} />
          ...
          {subtitlesEnabled && (curClip?.subtitles || []).length > 0 && (
            <button onClick={autoInsertMedia} disabled={autoMediaLoading}
              style={{ padding: "6px 14px", borderRadius: 8, border: "none", background: autoMediaLoading ? "rgba(0,0,0,0.06)" : "rgba(59,130,246,0.08)", color: autoMediaLoading ? "#3b82f6" : "#c084fc", cursor: autoMediaLoading ? "wait" : "pointer", fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", gap: 5 }}>
              {autoMediaLoading
                ? <><div style={{ width: 12, height: 12, borderRadius: "50%", border: "2px solid rgba(0,0,0,0.06)", borderTopColor: "#3b82f6", animation: "spin 0.8s linear infinite" }} /> AI 삽입 중...</>
