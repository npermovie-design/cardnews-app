            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ borderRadius:12,border:`1px solid ${bdr}`,background:cardBg,padding:"14px",marginBottom:10 }}>
                ...
                <div style={{ display:"flex",gap:10,marginBottom:12 }}>
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ display:"flex",gap:6,alignItems:"center" }}>
                      <input type="color" value={so.bgColor||curStyle.bgColor||"#1c1c1e"} onChange={e=>updSted(selIdx,"bgColor",e.target.value)} style={{ width:36,height:36,borderRadius:8,border:`1px solid ${bdr}`,cursor:"pointer",padding:2 }}/>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ display:"flex",gap:6,alignItems:"center" }}>
                      <input type="color" value={so.textColor||curStyle.textColor||"#ffffff"} onChange={e=>updSted(selIdx,"textColor",e.target.value)} style={{ width:36,height:36,borderRadius:8,border:`1px solid ${bdr}`,cursor:"pointer",padding:2 }}/>
                ...
                <div style={{ marginBottom:12 }}>
                  ...
                  <div style={{ display:"flex",gap:6,alignItems:"center" }}>
                    <button onClick={()=>updSted(selIdx,"titleSize",Math.max(16,(so.titleSize||curStyle.titleSize||32)-2))} style={{ ...btnSm,fontSize:18 }}>−</button>
                    ...
                    <button onClick={()=>updSted(selIdx,"titleSize",Math.min(72,(so.titleSize||curStyle.titleSize||32)+2))} style={{ ...btnSm,fontSize:18 }}>+</button>
                ...
                <div style={{ marginBottom:12 }}>
                  ...
                  <div style={{ display:"flex",gap:5 }}>
                    {[["left","왼쪽","⬅"],["center","가운데","↔"],["right","오른쪽","➡"]].map(([v,label,icon])=>{
                      ...
                      return <button key={v} onClick={()=>updSted(selIdx,"textAlign",v)} style={{ flex:1,padding:"7px 4px",borderRadius:8,border:`1px solid ${cur===v?"#6366f1":bdr}`,background:cur===v?"rgba(99,102,241,0.2)":"transparent",color:cur===v?"#a5b4fc":muted,cursor:"pointer",fontSize:11,fontWeight:cur===v?700:400,display:"flex",flexDirection:"column",alignItems:"center",gap:2 }}><span style={{ fontSize:14 }}>{icon}</span>{label}</button>;
                    })}
                  </div>
                </div>
                <div style={{ marginBottom:12 }}>
                  ...
                  <div style={{ display:"flex",gap:5 }}>
                    {[["top","상단","⬆"],["middle","가운데","↕"],["bottom","하단","⬇"]].map(([v,label,icon])=>{
                      ...
                      return <button key={v} onClick={()=>updSted(selIdx,"textValign",v)} style={{ flex:1,padding:"7px 4px",borderRadius:8,border:`1px solid ${cur===v?"#6366f1":bdr}`,background:cur===v?"rgba(99,102,241,0.2)":"transparent",color:cur===v?"#a5b4fc":muted,cursor:"pointer",fontSize:11,fontWeight:cur===v?700:400,display:"flex",flexDirection:"column",alignItems:"center",gap:2 }}><span style={{ fontSize:14 }}>{icon}</span>{label}</button>;
                    })}
                  </div>
                </div>
                <div>
                  ...
                  {so.bgImage ? (
                    ...
                    <button onClick={()=>bgFileRef.current?.click()} style={{ width:"100%",padding:"9px",borderRadius:8,border:`1.5px dashed ${bdr}`,background:"transparent",color:muted,fontSize:12,cursor:"pointer" }}>📸 배경 이미지 업로드</button>
                  )}
              ...
              <div style={{ display:"flex",gap:8 }}>
                <button onClick={()=>saveOne(selIdx)} style={{ flex:1,padding:"13px",borderRadius:11,border:"none",cursor:"pointer",background:"#6366f1",color:"#fff",fontSize:13,fontWeight:800 }}>📥 현재 PNG</button>
                <button onClick={saveAll} disabled={dlSt.busy} style={{ flex:1,padding:"13px",borderRadius:11,border:"none",cursor:"pointer",background:D?"rgba(255,255,255,0.1)":"#2c2c2c",color:"#fff",fontSize:13,fontWeight:800,opacity:dlSt.busy?0.7:1 }}>{dlSt.msg||"📦 전체 ZIP"}</button>
              </div>
            </div>
