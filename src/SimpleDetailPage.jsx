...
/* ══════════════════════════════════════════════════════════════
   ...
   - AI 이미지 생성 방식
   - 기본 사이즈: 세로형 860×1100
   - 상품명 + 핵심 내용만 입력하면 AI가 상세페이지 슬라이드 구성
...
export default function SimpleDetailPage({ isDark, user }) {
  ...
  // ── 보관함 ──────────────────────────────────────────────────
  const SAVES_KEY = "nper_simpledetail_saves_v1";
      ...
      <div style={{ flex:1, overflowY:"auto" }}>
        ...
        <div style={{ maxWidth:640, margin:"0 auto", padding:"0 28px 80px" }}>
          ...
          <div style={{ marginBottom:18 }}>
            <div style={{ fontSize:13, fontWeight:700, color:text, marginBottom:6 }}>어떤 상품/서비스인가요? *</div>
            <input
              value={topic}
              onChange={e=>setTopic(e.target.value)}
              placeholder="예: 프리미엄 한우 선물 세트"
              style={inputStyle}
            />
          </div>
          ...
          {/* 내용 */}
          <div style={{ marginBottom:18 }}>
            <div style={{ fontSize:13, fontWeight:700, color:text, marginBottom:6 }}>핵심 특징/내용을 알려주세요 *</div>
            <textarea
              value={content}
              onChange={e=>setContent(e.target.value)}
              placeholder="예: 1++ 등급, 냉장 당일 배송, 고급 선물 포장, 2인 세트 구성, 가격 89,000원"
              rows={4}
