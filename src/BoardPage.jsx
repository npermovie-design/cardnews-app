...
const BOARD_CSS = `
  ...
  .board-sidebar-desktop { display: none; }
  .board-sidebar-mobile { display: block; }
  .board-content { padding: 16px 14px; }
  .board-header { display: flex; flex-direction: column; align-items: flex-start; gap: 10px; }
  .board-write-btn { width: 100%; }
  .board-write-btn button { width: 100%; justify-content: center; }
  .postform-actions { display: flex; flex-direction: column; gap: 10px; }
  .postform-actions button { width: 100%; }
  .comment-input-row { display: flex; flex-direction: column; gap: 8px; }
  .comment-input-row button { width: 100%; }

  @media (min-width: 640px) {
    .board-layout { flex-direction: row; }
    .board-sidebar-desktop { display: flex; }
    .board-sidebar-mobile { display: none; }
    .board-content { padding: 32px 28px; }
    .board-header { flex-direction: row; align-items: center; gap: 0; }
    .board-write-btn { width: auto; }
    .board-write-btn button { width: auto; }
    .postform-actions { flex-direction: row; justify-content: flex-end; }
    .postform-actions button { width: auto; }
    .comment-input-row { flex-direction: row; }
    .comment-input-row button { width: auto; }
  }
`;
  ...
  return (
    <div style={{ fontSize: 14, color: C.text, lineHeight: 1.85, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
      {lines.map((line, i) => {
        ...
        if (ytMatch) return (
          <div key={i} style={{ margin: "12px 0", borderRadius: 12, overflow: "hidden", maxWidth: "100%" }}>
            <iframe src={"https://www.youtube.com/embed/" + ytMatch[1]}
              style={{ width: "100%", height: 220, border: "none" }} allowFullScreen title="youtube" />
          </div>
        );
    ...
    <div style={{ maxWidth: 700, width: "100%" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 10 }}>
        ...
        <button onClick={onCancel}
          style={{ background: "none", border: "1px solid " + C.border, color: C.muted, cursor: "pointer", fontSize: 13, padding: "6px 12px", borderRadius: 8 }}>취소</button>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div>
          ...
          <Textarea C={C} placeholder="내용을 입력해주세요" value={body} onChange={e => setBody(e.target.value)} rows={10} style={{ minHeight: 200 }} />
        </div>
        <div className="postform-actions">
          <button onClick={onCancel}
            style={{ padding: "10px 24px", borderRadius: 10, border: "1px solid " + C.border, background: "transparent", color: C.muted, fontSize: 14, cursor: "pointer", fontWeight: 600 }}>취소</button>
...
export default function BoardPage({ user, C }) {
  ...
  const submitPost = (form) => {
    ...
    const newPost = {
      id: Date.now(), cat, nick: user.nick, userId: user.id,
  ...
  const addComment = (postId) => {
    ...
    const next = posts.map(p => p.id === postId
      ? { ...p, comments: [...p.comments, { id: Date.now(), nick: user.nick, userId: user.id, text: comment, date: new Date().toLocaleDateString("ko-KR") }] }
      : p);
  ...
  /* ── 카테고리 네비 ── */
  const CatNav = ({ onSelect }) => (
    <>
      <style>{BOARD_CSS}</style>

      {/* 데스크톱: 세로 사이드바 */}
      <aside className="board-sidebar-desktop" style={{
        ...
        {CATS.map(cc => (
          <button key={cc.id}
            onClick={() => { setCat(cc.id); if (onSelect) onSelect(); }}
            style={{
              display: "flex", alignItems: "center", gap: 10,
              width: "100%", padding: "11px 18px",
              border: "none", cursor: "pointer", fontSize: 13, fontWeight: 600,
              background: cat === cc.id ? "rgba(124,106,255,0.08)" : "transparent",
      ...
      {/* 모바일: 가로 탭 */}
      <div className="board-sidebar-mobile" style={{
  ...
  if (editing || writing) return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "calc(100vh - 64px)" }}>
      ...
      <div className="board-layout">
        <div className="board-content" style={{ flex: 1, minWidth: 0 }}>
  ...
  if (view) return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "calc(100vh - 64px)" }}>
      <CatNav onSelect={() => setView(null)} />
      <div className="board-layout">
        <div className="board-content" style={{ flex: 1, minWidth: 0, maxWidth: "100%" }}>

          <button onClick={() => setView(null)}
            style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontSize: 13, marginBottom: 16, padding: 0, fontWeight: 600 }}>
            ← 목록으로
          </button>

          <div style={{
            background: C.card, border: "1px solid " + C.border,
            borderRadius: 16, padding: "clamp(16px,3vw,28px)", marginBottom: 14, boxShadow: C.shadow,
          }}>
              ...
              {isOwner(view) && (
                <div style={{ display: "flex", gap: 6 }}>
                  <button onClick={() => setEditing(view)}
                    style={{ padding: "4px 12px", borderRadius: 7, border: "1px solid " + C.border, background: "transparent", color: C.muted, fontSize: 11, cursor: "pointer" }}>수정</button>
                  <button onClick={() => deletePost(view.id)}
                    style={{ padding: "4px 12px", borderRadius: 7, border: "1px solid rgba(229,62,62,0.2)", background: "rgba(229,62,62,0.05)", color: "#e53e3e", fontSize: 11, cursor: "pointer" }}>삭제</button>
                </div>
              )}
            </div>
            <h2 style={{ color: C.text, fontSize: "clamp(16px,4vw,20px)", fontWeight: 800, margin: "0 0 16px", lineHeight: 1.4, wordBreak: "keep-all" }}>{view.title}</h2>
                ...
                {editComment && editComment.idx === i ? (
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    ...
                    <button onClick={() => submitEditComment(view.id)}
                      style={{ padding: "6px 12px", borderRadius: 8, border: "none", background: C.purple, color: "#fff", fontSize: 12, cursor: "pointer" }}>저장</button>
                    <button onClick={() => setEditComment(null)}
                      style={{ padding: "6px 12px", borderRadius: 8, border: "1px solid " + C.border, background: "transparent", color: C.muted, fontSize: 12, cursor: "pointer" }}>취소</button>
                  </div>
                ) : (
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <span style={{ color: C.purpleL, fontWeight: 600, fontSize: 13, marginRight: 8 }}>{c.nick}</span>
                      <span style={{ fontSize: 13, color: C.text, lineHeight: 1.7, wordBreak: "break-word" }}>{c.text}</span>
                    ...
                    {user && (user.nick === c.nick || user.role === "admin") && (
                      <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                        <button onClick={() => setEditComment({ idx: i, text: c.text })}
                          style={{ padding: "3px 9px", borderRadius: 6, border: "1px solid " + C.border, background: "transparent", color: C.muted, fontSize: 10, cursor: "pointer" }}>수정</button>
                        <button onClick={() => deleteComment(view.id, i)}
                          style={{ padding: "3px 9px", borderRadius: 6, border: "1px solid rgba(229,62,62,0.2)", background: "transparent", color: "#e53e3e", fontSize: 10, cursor: "pointer" }}>삭제</button>
                      </div>
                    )}
                  </div>
                )}
          ...
          {user ? (
            <div className="comment-input-row">
              <Inp C={C} placeholder="댓글을 입력해주세요..." value={comment}
                onChange={e => setComment(e.target.value)}
                onKeyDown={e => e.key === "Enter" && addComment(view.id)}
                style={{ flex: 1 }} />
              <button onClick={() => addComment(view.id)}
                style={{ padding: "10px 18px", borderRadius: 10, border: "none", background: C.purple, color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", flexShrink: 0 }}>등록</button>
            </div>
          ) : (
            <div style={{ fontSize: 13, color: C.muted, textAlign: "center", padding: "14px", background: C.purpleBg, borderRadius: 10, border: "1px solid rgba(124,106,255,0.15)" }}>
              댓글은 로그인 후 이용 가능합니다
            </div>
          )}
        </div>
      ...
      <div className="board-layout">
        <div className="board-content" style={{ flex: 1, minWidth: 0 }}>
          ...
          <div className="board-header" style={{ marginBottom: 20 }}>
            <div style={{ flex: 1 }}>
              <h2 style={{ color: C.text, fontSize: 18, fontWeight: 900, margin: "0 0 4px" }}>
          ...
          {filtered.length === 0 ? (
            ...
            <div key={p.id}
              style={{
                ...
                boxShadow: C.shadow, transition: "box-shadow 0.15s,border-color 0.15s", cursor: "pointer",
              }}
              onClick={() => setView(p)}
              ...
              <div style={{ fontSize: 18, opacity: 0.2, color: C.text, flexShrink: 0, marginLeft: 10 }}>›</div>
            </div>
          ))}
        </div>
      </div>
