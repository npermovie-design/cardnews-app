...
══════════════════════════════════════════════════ */
const BOARD_CSS = `
  .bw { display:flex; flex-direction:column; min-height:calc(100vh - 64px); }
  .bl { display:flex; flex:1; flex-direction:column; }
  .bs { display:none; }
  .bmt { display:block; }
  ...
  .dp  { padding:14px; }
  .fp  { padding:14px; }
  .pfa { flex-direction:column; }
  .pfa>* { width:100%; }
  .hr  { flex-direction:column; align-items:flex-start!important; gap:10px; padding:12px 14px; }
  .wb  { width:100%; }
  .wb button { width:100%; justify-content:center; }

  @media(min-width:640px){
    .bl { flex-direction:row; }
    .bs { display:flex; }
    .bmt { display:none; }
    .pth { display:flex; }
    ...
    .fp  { padding:22px 28px; }
    .pfa { flex-direction:row; justify-content:flex-end; }
    .pfa>* { width:auto; }
    .hr  { flex-direction:row; align-items:center!important; gap:0; padding:16px 20px; }
    .wb  { width:auto; }
...
function PostForm({ user, cat, initial, onSubmit, onCancel, C }) {
  ...
  const insertLine = (text) => setBody(b => b + (b.endsWith("\n")||!b ? "" : "\n") + text + "\n");
  ...
  return (
    <div className="fp" style={{ maxWidth:900, width:"100%", boxSizing:"border-box" }}>
        ...
        {showImg && (
          <div style={{ padding:"10px 14px", background:C.bg2, borderBottom:"1px solid "+C.border, display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
            ...
            <input value={imgUrl} onChange={e=>setImgUrl(e.target.value)} placeholder="https://example.com/image.jpg"
              ...
              style={{ flex:1, minWidth:200, border:"1px solid "+C.border, outline:"none", padding:"7px 10px", fontSize:13, background:C.card, color:C.text, borderRadius:6, fontFamily:"inherit" }}/>
            <button onClick={()=>{ if(imgUrl.trim()){ insertLine(imgUrl.trim()); setImgUrl(""); setShowImg(false); }}} style={{ padding:"7px 16px", borderRadius:6, border:"none", background:"#6366f1", color:"#fff", fontSize:12, fontWeight:700, cursor:"pointer" }}>삽입</button>
      ...
      {/* 버튼 */}
      <div className="pfa" style={{ display:"flex", gap:8, marginTop:14 }}>
        <button onClick={onCancel}
...
export default function BoardPage({ user, C, onLoginRequest, lang="ko", t: tProp, initialCat, embedded }) {
  ...
  const requireLogin = () => { if (onLoginRequest) onLoginRequest(); };
  ...
  const CatNav = () => embedded ? null : (
    <>
      ...
      {/* 데스크톱 사이드바 */}
      <aside className="bs" style={{ width:160, flexShrink:0, borderRight:"1px solid "+B, background:C.sidebarBg, flexDirection:"column", paddingTop:4 }}>
      ...
      {/* 모바일 가로 탭 */}
      <div className="bmt" style={{ borderBottom:"1px solid "+B, background:C.sidebarBg, overflowX:"auto", WebkitOverflowScrolling:"touch" }}>
  ...
  if (view) return (
    <div className="bw">
      <CatNav/>
      <div className="bl">
        <div className="bm dp" style={{ maxWidth:900, boxSizing:"border-box" }}>
                ...
                {isOwner(view)&&(
                  <div style={{ marginLeft:"auto", display:"flex", gap:6 }}>
                    <button onClick={()=>setEditing(view)}
                      style={{ padding:"3px 11px", borderRadius:5, border:"1px solid "+B, background:"transparent", color:C.muted, fontSize:11, cursor:"pointer" }}>수정</button>
                    <button onClick={()=>deletePost(view.id)}
                      style={{ padding:"3px 11px", borderRadius:5, border:"1px solid rgba(229,62,62,0.3)", background:"rgba(229,62,62,0.05)", color:"#e53e3e", fontSize:11, cursor:"pointer" }}>삭제</button>
                  </div>
                )}
              </div>
            </div>
          ...
          <div style={{ display:"flex", gap:6, padding:"10px 0", borderBottom:"1px solid "+B, marginBottom:18 }}>
            <button onClick={()=>setView(null)}
              style={{ padding:"7px 18px", borderRadius:6, border:"1px solid "+B, background:BG, color:C.text, fontSize:13, cursor:"pointer", fontWeight:600 }}>목록</button>
            {isOwner(view)&&<>
              <button onClick={()=>setEditing(view)}
                style={{ padding:"7px 18px", borderRadius:6, border:"1px solid "+B, background:BG, color:C.text, fontSize:13, cursor:"pointer" }}>수정</button>
              <button onClick={()=>deletePost(view.id)}
              ...
              <div key={c.id||i} style={{ borderBottom:"1px solid "+B, padding:"11px 14px" }}>
                {editCmt&&editCmt.idx===i?(
                  <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                    <Inp C={C} value={editCmt.text}
                      onChange={e=>setEditCmt({...editCmt,text:e.target.value})}
                      onKeyDown={e=>e.key==="Enter"&&submitEditCmt(view.id)}
                      style={{ flex:1, minWidth:120 }}/>
                    <button onClick={()=>submitEditCmt(view.id)}
                      style={{ padding:"7px 13px", borderRadius:6, border:"none", background:C.purple, color:"#fff", fontSize:12, cursor:"pointer" }}>저장</button>
                    <button onClick={()=>setEditCmt(null)}
                      style={{ padding:"7px 12px", borderRadius:6, border:"1px solid "+B, background:"transparent", color:C.muted, fontSize:12, cursor:"pointer" }}>취소</button>
                  </div>
                ):(
                  <div>
                    <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:5, flexWrap:"wrap" }}>
                      ...
                      {user&&(user.nick===c.nick||user.role==="admin")&&(
                        <div style={{ marginLeft:"auto", display:"flex", gap:4 }}>
                          <button onClick={()=>setEditCmt({idx:i,text:c.text})}
                            style={{ padding:"2px 7px", borderRadius:4, border:"1px solid "+B, background:"transparent", color:C.muted, fontSize:10, cursor:"pointer" }}>수정</button>
                          <button onClick={()=>deleteCmt(view.id,i)}
                            style={{ padding:"2px 7px", borderRadius:4, border:"1px solid rgba(229,62,62,0.2)", background:"transparent", color:"#e53e3e", fontSize:10, cursor:"pointer" }}>삭제</button>
                        </div>
                      )}
                    </div>
    ...
    <div className="bw">
      ...
      <div className="bl">
        <div className="bm">
          ...
          {/* 게시판 헤더 */}
          <div className="hr" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", borderBottom:"2px solid "+C.text }}>
            <div>
          ...
          {/* 테이블 헤더 (PC) */}
          <div className="pth" style={{ display:"flex", background:BG, borderBottom:"1px solid "+B, fontSize:12, fontWeight:700, color:C.muted }}>
              ...
              <div key={p.id} className="ptr"
                ...
                <div style={{ flex:1, padding:"9px 12px", minWidth:0 }}>
                  ...
                  <div className="ptm" style={{ color:C.muted, marginTop:2 }}>
                    <span>{p.nick}</span>
                    <span className="ptds">{p.date}</span>
                ...
                <div className="ptx" style={{ width:90, padding:"11px 0", textAlign:"center", fontSize:12, color:C.muted, flexShrink:0 }}>
                  <span className="ptdf">{p.date}</span>
                </div>
