...
const FILE_TYPE_INFO = {
  ...
  other:   { icon: "📦", label: "파일",   color: "#7c6aff" },
};
        ...
        {fileType === "other" && (
          <div style={{ textAlign: "center", padding: "40px 20px" }}>
            ...
            <a href={fileUrl} download target="_blank" rel="noopener noreferrer"
              style={{ padding: "12px 32px", borderRadius: 10, background: "linear-gradient(135deg,#7c6aff,#8b5cf6)", color: "#fff", fontSize: 14, fontWeight: 700, textDecoration: "none" }}>
              ⬇️ 다운로드
            </a>
          </div>
        )}
        ...
        <div style={{ position: "absolute", inset: 0 }}>
          ...
          {isAdmin && hovered && !playing && (
            <div style={{ position: "absolute", bottom: 10, right: 10, display: "flex", gap: 5, zIndex: 5 }}>
              <button onClick={e => { e.stopPropagation(); onEdit(item); }} style={{ padding: "6px 12px", borderRadius: 8, border: "none", background: "rgba(255,255,255,0.92)", color: "#7c6aff", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>✏️</button>
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 3 }}>
          <span style={{ fontSize: 10, padding: "2px 7px", borderRadius: 4, background: (typeInfo.color || "#7c6aff") + "22", color: typeInfo.color || "#7c6aff", fontWeight: 700 }}>{typeInfo.label}</span>
      ...
      <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
        <button onClick={handleDownload} disabled={downloading} style={{
          ...
          background: "transparent", color: isDark ? "#a5b4fc" : "#7c6aff",
    ...
    <div style={{ maxWidth: 660, padding: "28px 0 60px" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
          {[["file","📁 파일 직접 업로드"],["url","🔗 URL로 등록"]].map(([m, l]) => (
            <button key={m} onClick={() => setMode(m)} style={{
              ...
              border: `2px solid ${mode === m ? "#7c6aff" : bdr}`,
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        {uploading && (
          <div style={{ padding: "14px 16px", borderRadius: 10, background: "rgba(99,102,241,0.08)", border: "1px solid rgba(99,102,241,0.2)" }}>
            ...
            <div style={{ height: 6, borderRadius: 6, background: "rgba(255,255,255,0.1)", overflow: "hidden" }}>
              <div style={{ height: "100%", borderRadius: 6, width: progress + "%", background: "linear-gradient(90deg,#7c6aff,#8b5cf6)", transition: "width 0.3s" }} />
            </div>
          </div>
        )}
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <button type="button" onClick={() => thumbRef.current?.click()} style={{
              ...
              border: `1px dashed ${thumbPreview ? "#7c6aff" : bdr}`,
        ...
        <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", padding: "12px 14px", borderRadius: 10, background: cardBg, border: `1px solid ${bdr}` }}>
          <input type="checkbox" checked={form.isFree} onChange={e => setForm(p => ({ ...p, isFree: e.target.checked }))} style={{ width: 16, height: 16, accentColor: "#7c6aff" }} />
          <div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "20px 20px 0" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            ...
            {isAdmin && (
              <button onClick={() => setMenu("upload")} style={{ padding: "7px 14px", borderRadius: 9, border: "none", background: "linear-gradient(135deg,#7c6aff,#8b5cf6)", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>⬆️ 업로드</button>
            )}
          </div>
        </div>
        ...
        {!loading && filtered.length === 0 && (
          <div style={{ textAlign: "center", padding: "80px 0", color: muted }}>
            ...
            {isAdmin && !search && (
              <button onClick={() => setMenu("upload")} style={{ marginTop: 12, padding: "10px 24px", borderRadius: 10, border: "none", background: "linear-gradient(135deg,#7c6aff,#8b5cf6)", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>⬆️ 첫 파일 업로드하기</button>
            )}
          </div>
        )}
            ...
            {filtered.map((v, idx) => {
              ...
              return (
                <div key={v.key} style={{ display: "grid", gridTemplateColumns: "44px 56px 1fr 80px 70px 110px",
                  ...
                  <span style={{ fontSize: 10, padding: "2px 7px", borderRadius: 4, background: (ti.color||"#7c6aff")+"22", color: ti.color||"#7c6aff", fontWeight: 700, whiteSpace: "nowrap" }}>{ti.label}</span>
          ...
          {CATEGORIES.map(c => (
            <button key={c.id} onClick={() => { setCat(c.id); setMenu("files"); }}
              style={{ display: "flex", alignItems: "center", gap: 6, padding: "13px 18px", border: "none", cursor: "pointer",
                ...
                color: cat === c.id ? (c.color || "#7c6aff") : (isDark ? "rgba(255,255,255,0.45)" : "#888"),
                borderBottom: cat === c.id ? `2px solid ${c.color || "#7c6aff"}` : "2px solid transparent",
              }}>
              {c.icon} {c.label}
