...
const PAGE_META = {
  ...
  class: { title: "클래스 - SNS메이킷", description: "SNS 콘텐츠 제작, 자동화, 마케팅 실무를 배우는 SNS메이킷 클래스입니다." },
  challenge: { title: "성장 프로그램 - SNS메이킷", description: "SNS 성장 목표를 정하고 매일 인증하며 실행하는 SNS메이킷 성장 프로그램입니다." },
  growth: { title: "성장 프로그램 - SNS메이킷", description: "SNS 성장 목표를 정하고 매일 인증하며 실행하는 SNS메이킷 성장 프로그램입니다." },
  newsletter: { title: "뉴스레터 - SNS메이킷", description: "SNS 마케팅 최신 트렌드와 인사이트를 매주 받아보세요." },
  pricing: { title: "가격정책 - SNS메이킷", description: "SNS메이킷 가격정책. 가입 시 5회 지급, Basic 월 30회, Pro 월 200회, Business 월 700회 제공. 영상 생성은 Pro부터 제한 없이 사용할 수 있습니다." },
...
export default async function handler(req) {
  ...
  const htmlLang = langParam === "zh" ? "zh-CN" : ["ko", "en", "ja"].includes(langParam) ? langParam : "ko";
  const altBasePath = canonicalPath === "/" ? "" : canonicalPath;
...
<link rel="alternate" hreflang="ko" href="${SITE_URL}${altBasePath}">
<link rel="alternate" hreflang="en" href="${SITE_URL}${altBasePath}?lang=en">
<link rel="alternate" hreflang="ja" href="${SITE_URL}${altBasePath}?lang=ja">
<link rel="alternate" hreflang="zh-CN" href="${SITE_URL}${altBasePath}?lang=zh">
<link rel="alternate" hreflang="x-default" href="${SITE_URL}${altBasePath}">
